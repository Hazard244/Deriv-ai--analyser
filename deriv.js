const Deriv = (() => {
  ...
  const state = {
    ...
    reconnects: 0,
    loadedSymbols: [],
    duplicateHistoryResponses: 0,
    historyResponses: 0,
    subscriptionSymbols: CONFIG.MARKETS.map(m => m.symbol)
  };
  ...
  function updateState() {
    ...
    state.connectedMarkets = Object.values(state.liveByMarket).filter(count => count > 0).length;
  ...
  function resetState() {
    ...
    state.loadedSymbols = [];
    state.duplicateHistoryResponses = 0;
    state.historyResponses = 0;
    state.subscriptionSymbols = CONFIG.MARKETS.map(m => m.symbol);
  ...
  function handleHistory(data) {
    ...
    if (typeof request === "string" && request.startsWith("history:")) {
      ...
      const requestedSymbol = request.slice("history:".length);
      // Count each configured market exactly once. Deriv/reconnect races can
      // occasionally deliver duplicate history responses; those must never
      // turn 10/10 into 20/10.
      if (!state.loadedSymbols.includes(requestedSymbol)) {
        state.loadedSymbols.push(requestedSymbol);
        state.marketsLoaded = state.loadedSymbols.length;
      } else {
        state.duplicateHistoryResponses++;
      }
  ...
  function handleTick(data) {
    ...
    // A stale/transient error from a previous cached request should not
    // remain visible once the current connection is demonstrably healthy.
    if (state.marketsLoaded >= CONFIG.MARKETS.length && state.ticksReceived > 0) {
      state.error = null;
    }
