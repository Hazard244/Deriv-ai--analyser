    ...
    if (gateBox) {
      gateBox.innerHTML = gate ? `
        <div class="validation-card">
          ...
          <span>Calibrated probability: ${fmt(gate.calibratedProbability)}%</span>
          <span>Walk-forward: ${gate.validatedAccuracy == null ? "--" : fmt(gate.validatedAccuracy)+"%"} • Signals: ${gate.validatedSignals}</span>
          <span>Reliability: ${fmt(gate.calibrationReliability)} • ECE: ${gate.calibrationECE == null ? "--" : fmt(gate.calibrationECE,4)}</span>
          <span class="${gate.actionable ? "good" : "warn"}">${gate.verdict}</span>
          ${gate.blockers?.length ? `<span class="bad">Blockers: ${gate.blockers.join(" • ")}</span>` : ""}
