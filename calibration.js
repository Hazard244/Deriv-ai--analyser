const Calibration = (() => {
  ...
  const RELIABILITY_TARGET_SAMPLES = 100;
  ...
  function metricSummary(records, probabilityField="calibrated") {
    ...
    return {
      ...
      reliabilityBand:reliabilityBand(reliabilityFor(records.length,ece),records.length)
    };
  }
  ...
  // V6.4: reliability is an evidence-quality score, not a direct copy of
  // sample count. V6.3's linear /200 sample factor made 100+ observations
  // look artificially weak even when calibration error was only moderate.
  // We now reach full sample evidence at ~100 observations while still
  // penalizing poor calibration error. This keeps the score conservative
  // without permanently suppressing mature models.
  function reliabilityFor(samples,ece) {
    if (!samples) return 0;
    const evidence = Math.min(1, Math.sqrt(samples / RELIABILITY_TARGET_SAMPLES));
    const fit = Math.max(0, 1 - (Number(ece || 0) / 0.15));
    return Number((100 * evidence * fit).toFixed(1));
  }

  function reliabilityBand(reliability,samples) {
    if (!samples) return "UNCALIBRATED";
    if (reliability >= 70 && samples >= 150) return "STRONG";
    if (reliability >= 50 && samples >= 75) return "USABLE";
    if (reliability >= 30 && samples >= 30) return "DEVELOPING";
    return "WEAK";
  }
  ...
  function calibrate(symbol,confidence,regime="UNKNOWN") {
    ...
    return {
      ...
      reliabilityBand:model.reliabilityBand,
      brier:model.brier,
      ece:model.ece,
      regime
    };
  ...
  window.addEventListener("deriv:history",() => {
    clearTimeout(window.__v64CalibrationTimer);
    window.__v64CalibrationTimer=setTimeout(run,180);
  });
  window.addEventListener("scanner:update",() => {
    if (Date.now()-(state.lastUpdate||0)>15000) {
      clearTimeout(window.__v64CalibrationTimer);
      window.__v64CalibrationTimer=setTimeout(run,50);
    }
  });
