const Scanner = (() => {
  ...
  function push(symbol, tick) {
    ...
    book.push(tick);
    if (book.length > CONFIG.MAX_HISTORY) {
      book.splice(0, book.length - CONFIG.MAX_HISTORY);
    }
  }
  ...
  function windowStats(book, n) {
    ...
    if (!a.length) {
      return { samples: 0, even: 50, odd: 50, edge: 0, direction: "NEUTRAL" };
    }
    ...
    return {
      ...
      odd: oddPct,
      edge,
      direction: evenPct > oddPct ? "EVEN" : oddPct > evenPct ? "ODD" : "NEUTRAL"
    };
  }
  ...
  function markov(book, n = 250) {
    ...
    return {
      ...
      even,
      odd,
      edge: Math.abs(even - odd),
      direction: even > odd ? "EVEN" : odd > even ? "ODD" : "NEUTRAL"
    };
  }
  ...
  function switchRate(book, n = 100) {
    ...
    let switches = 0;
    for (let i = 1; i < a.length; i++) {
      if ((a[i].digit % 2) !== (a[i - 1].digit % 2)) switches++;
    }
    return (switches / (a.length - 1)) * 100;
  }

  function multiWindow(book) {
    const windows = CONFIG.WINDOWS.map(n => ({
      n,
      ...windowStats(book, n)
    }));
    ...
    const direction = evenVotes > oddVotes ? "EVEN" :
      oddVotes > evenVotes ? "ODD" : "NEUTRAL";
  ...
  function scoreMarket(market) {
    ...
    const multi = multiWindow(book);
    const short = windowStats(book, 50);
    const medium = windowStats(book, 250);
    const long = windowStats(book, 1000);
    const mk = markov(book);
    const ent = entropy(book);
    const switches = switchRate(book);
    ...
    const winner = evenScore > oddScore ? "EVEN" :
      oddScore > evenScore ? "ODD" : "NEUTRAL";
    ...
    // Penalize evidence that looks too close to random.
    if (ent > 0.985) opportunity -= 12;
    else if (ent > 0.95) opportunity -= 6;

    // Penalize poor multi-window agreement.
    if (multi.alignment < 60) opportunity -= 8;

    // Markov gets less influence when transition edge is tiny.
    if (mk.edge < 3) opportunity -= 3;

    // A very extreme switch rate can indicate rapid alternation/noisy conditions.
    ...
    const confidence = clamp(
      opportunity * 0.85 +
      multi.alignment * 0.10 +
      Math.min(5, separation * 0.10)
    );
    ...
    return {
      ...
      alignment: Number(multi.alignment.toFixed(0)),
      markovEdge: Number(mk.edge.toFixed(1)),
      shortEven: Number(short.even.toFixed(1)),
      shortOdd: Number(short.odd.toFixed(1)),
      switchRate: Number(switches.toFixed(1)),
      reason: reasons.join(" • ") || "Insufficient edge."
    };
  }

  function scan() {
    const results = CONFIG.MARKETS
      .map(scoreMarket)
      .sort((a, b) => b.opportunity - a.opportunity);

    const top3 = results.slice(0, 3).map((market, index) => ({
      rank: index + 1,
      ...market
    }));
  ...
  function start() {
    if (timer) return;
    timer = setInterval(scan, CONFIG.SCAN_INTERVAL_MS);
    scan();
  }
  ...
  window.addEventListener("deriv:history", event => {
    load(event.detail.symbol, event.detail.ticks);
    scan();
  });

  window.addEventListener("deriv:tick", event => {
    push(event.detail.symbol, event.detail);
    // Re-rank immediately when a real live tick arrives.
    scan();
  });
  ...
  return {
    ...
    stop,
    scan,
    getSnapshot: () => lastScan,
    getMarketHistory: symbol => [...(books.get(symbol) || [])]
  };
})();
