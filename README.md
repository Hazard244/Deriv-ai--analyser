...
## V6.2 prequential calibration hardening
V6.2 fixes a critical calibration-validity issue: V6.1 could fit reliability bins from the
same walk-forward records that it then used to report calibration quality. That can make
ECE/Brier look better than true out-of-sample performance.

V6.2 therefore evaluates each historical prediction prequentially. A prediction can only
use calibration records that existed before that prediction. The final live calibration
model is still trained from accumulated historical records, but the displayed quality,
ECE, Brier score and reliability are based on the leakage-resistant prequential evaluation.

Regime-specific calibration remains enabled, with a minimum evidence threshold before a
regime model is allowed to override the market model.

## V6.4 evidence-based decision gate
V6.4 adds a final decision layer between raw opportunity ranking and the displayed actionable signal.
The gate evaluates:
- walk-forward accuracy and validated signal count
- calibrated probability
- calibration reliability and ECE
- multi-window alignment
- Markov edge and ranking separation
- entropy and switch-rate noise
- opportunity score only as a secondary ranking input

A high opportunity score cannot override poor historical validation. The gate can return
WAIT, EARLY, CONFIRMED or HIGH. EARLY/CONFIRMED/HIGH are actionable labels for analysis only;
this build remains read-only and does not place trades.
